...
5. **デバイス** のドロップダウンから `Live Gamer EXTREME 3` を選択する
   - このタイミングで macOS のカメラアクセス許可ダイアログが表示された場合は **OK** をクリックして許可する
6. 以下の設定を行う：
   - **解像度**: `1920x1080`（Switch2 のドック出力は最大 1080p のため、この解像度で問題ない）
   - **フレームレート**: `60` fps
   - **ピクセルフォーマット**: `NV12` を推奨（`YUY2` も選択可能だが、`NV12` の方が GPU 処理が効率的で帯域幅も少ない）
7. **OK** をクリックする

> **ポイント:** GC551G2 が正しく接続されていれば、デバイス一覧に `Live Gamer EXTREME 3` と表示される。
...
4. **デバイス** のドロップダウンから `Live Gamer EXTREME 3` を選択する
5. **OK** をクリックする

または、**設定** → **音声** にある **補助音声** に `Live Gamer EXTREME 3` を設定する方法もある。
...
| 症状 | 確認事項 |
| --- | --- |
| OBS のソース一覧に Live Gamer EXTREME 3 が表示されない | USB ケーブルの再接続、macOS のカメラ権限の確認 |
